namespace Z0
{
    ...
    partial class Build
    {
        ...
        public class ProjectWorkspace : IDisposable
        {
            ...
            readonly Dictionary<string, E.ProjectItem> _documents = new(StringComparer.OrdinalIgnoreCase);
            ...
            {
                var projectId = Microsoft.CodeAnalysis.ProjectId.CreateNewId(debugName: projectFileInfo.FilePath.Format());

                var language = projectFileInfo.Language;
                var projectPath = projectFileInfo.FilePath;

                var projectName = projectPath.WithoutExtension.Format();
                ...
                var version = VersionStamp.Create(
                    FileUtilities.GetFileTimeStamp(projectPath.Name));
                ...
                static CompilationOptions GetCompilationOptions(bool isCSharp) =>
                    CSharpDefaults.CompilationOptions;
                ...
                var documents = CreateDocumentInfos(projectFileInfo.Documents.Array(), projectId, commandLineArgs.Encoding);
                var additionalDocuments = CreateDocumentInfos(projectFileInfo.AdditionalDocuments.Array(), projectId, commandLineArgs.Encoding);
                var analyzerConfigDocuments = CreateDocumentInfos(projectFileInfo.AnalyzerConfigDocuments.Array(), projectId, commandLineArgs.Encoding);
                ...
                return ProjectInfo.Create(
                    ...
                    language,
                    projectPath.Format(),
                    outputFilePath: projectFileInfo.OutputFilePath.Format(),
                    outputRefFilePath: projectFileInfo.OutputRefFilePath.Format(),
                ...
                MSBProject loadedProject, string language, string projectDirectory, CancellationToken cancellationToken)
            {
                ...
                return project != null
                    ...
                    : ProjectFileInfo.CreateEmpty(language, FS.path(loadedProject.FullPath));
            }
                ...
                ProjectInstance projectInstance, MSBProject loadedProject, string language, string projectDirectory)
            {
                ...
                return ProjectFileInfo.Create(
                    language,
                    FS.path(projectInstance.FullPath),
                    FS.path(outputFilePath),
                    FS.path(outputRefFilePath),
                    defaultNamespace,
                    targetFramework ?? "<unknown>",
                    commandLineArgs.Array(),
                    docs.Array(),
                    additionalDocs.Array(),
                    ReadOnlySeq<DocumentFileInfo>.Empty, //analyzerConfigDocs,
                    ...
                    .Select(i =>
                    {
                        ...
                        return new ProjectFileReference(FS.path(i.EvaluatedInclude), aliases.Array());
                    }).ToSeq());
            }
                ...
                MSBProject project, ITaskItem documentItem, string projectDirectory)
            {
                ...
                return new DocumentFileInfo(FS.path(filePath), logicalPath, isLinked, isGenerated, SourceCodeKind.Regular);
            }
            ...
            static string GetAssemblyNameFromProjectPath(FilePath projectFilePath)
            {
                var assemblyName = projectFilePath.WithoutExtension.Format();
            ...
            static ImmutableArray<DocumentInfo> CreateDocumentInfos(
                IReadOnlyList<DocumentFileInfo> documentFileInfos, Microsoft.CodeAnalysis.ProjectId projectId, Encoding? encoding)
            {
                ...
                foreach (var info in documentFileInfos)
                {
                    ...
                    var documentInfo = DocumentInfo.Create(
                        DocumentId.CreateNewId(projectId, debugName: info.FilePath.Format()),
                        name,
                        folders,
                        info.SourceCodeKind,
                        new FileTextLoader(info.FilePath.Format(), encoding),
