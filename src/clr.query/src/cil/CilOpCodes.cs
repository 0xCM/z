...
public class CilOpCodes
{
    ...
    public static ReadOnlySpan<CilOpCode> View
       => slice(_OpCodes.View, 0, OpCodeCount);    

    static readonly Seq<CilOpCode> _OpCodes;

    static readonly uint OpCodeCount;

    static CilOpCodes()
    {
        _OpCodes = alloc<CilOpCode>(300);
        OpCodeCount = load(ref _OpCodes.First);        
    }

    [MethodImpl(Inline), Op]
    public static CilOpCode pack(C id, string name, CT type, OT optype, byte opcount, ushort value, SB sb1, SB sb2)
        => new (id, value, name, type, optype, opcount, sb1, sb2);
