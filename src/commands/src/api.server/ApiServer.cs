...
namespace Z0
{
    ...
    public abstract class ApiServer : AppService
    {
        ...
        public static IApiServerContext<C> context<C>(IWfRuntime wf, Func<ReadOnlySeq<IApiCmdProvider>> factory)
            where C : IApiService<C>, new()
        {
        ...
        static IApiServerContext<C> context<C>(IWfRuntime wf, IWfChannel channel, ReadOnlySeq<IApiCmdProvider> providers)
            where C : IApiService<C>, new()
        {
            ...
            service.Init(wf);
            var context = new ApiServerContext<C>(service, channel, dispatcher(service, channel, providers));
