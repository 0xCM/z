...
public record struct AsmOperand : IAsmOp
{
    ...
    [MethodImpl(Inline)]
    public static implicit operator AsmOperand(Rel8 src)
        => new (src);

    [MethodImpl(Inline)]
    public static implicit operator AsmOperand(Rel16 src)
        => new (src);

    [MethodImpl(Inline)]
    public static implicit operator AsmOperand(Rel32 src)
        => new (src);
