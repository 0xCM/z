...
namespace Z0.Asm
{
    [ApiHost("asm.prototypes.pointers")]
    public unsafe readonly struct AsmPointers
    {
        ...
        public static void f_pc128u_pc128u_pc128u_void(Cell128* pA, Cell128* pB, Cell128* pDst)
        {
            ...
            pDst[i++] = vgcpu.vand<byte>(pA[i], pB[i]);
            ...
            pDst[i++] = vgcpu.vor<byte>(pA[i], pB[i]);
            ...
            pDst[i++] = vgcpu.vxor<byte>(pA[i], pB[i]);
            ...
            pDst[i++] = vgcpu.vsub<byte>(pA[i], pB[i]);
            ...
            pDst[i++] = vgcpu.vadd<byte>(pA[i], pB[i]);
            // 00adh add rsp,58h
            // 00b1h ret
        }
