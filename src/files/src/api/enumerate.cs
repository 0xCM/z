...
namespace Z0
{
    partial struct FS
    {
        ...
        public static IEnumerable<FilePath> enumerate(FolderPath root, FileKind kind, bool recurse)
            => enumerate(root, recurse, kind.Ext());

        [MethodImpl(Inline), Op]
        public static IEnumerable<FilePath> enumerate(FolderPath src, bool recurse, FileExt ext)
            => files(src, ext.SearchPattern, recurse);

        public static IEnumerable<FilePath> enumerate(FolderPath src, string pattern, bool recurse)
        {
            if(src.Exists)
        ...
        public static IEnumerable<FilePath> enumerate(FolderPath src, bool recurse, params FileKind[] kinds)
        {
            if(src.Exists)
            {
        ...
        public static IEnumerable<FilePath> enumerate(FolderPath src, bool recurse, params FileExt[] extensions)
        {
            if(src.Exists)
            {
