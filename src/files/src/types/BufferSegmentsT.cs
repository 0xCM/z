...
namespace Z0
{
        ...
        where T : unmanaged
    {
        ...
        {
            Buffer = buffer;
            Ranges = span<Pair<uint>>(max);
            Dispensed = 0;
        }

        [MethodImpl(Inline)]
        public BufferSegments(Span<T> buffer, Span<Pair<uint>> ranges)
        {
            Buffer = buffer;
            Ranges = ranges;
            Dispensed = 0;
        ...
        [MethodImpl(Inline)]
        public ref Pair<uint> Range(byte index)
            => ref seek(Ranges,index);

        [MethodImpl(Inline)]
        public ref Pair<uint> Range(uint index)
            => ref seek(Ranges,index);
