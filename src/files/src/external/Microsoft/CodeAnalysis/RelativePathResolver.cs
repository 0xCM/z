...
namespace Microsoft.CodeAnalysis
{
    ...
    public class RelativePathResolver : IEquatable<RelativePathResolver>
    {
        ...
        /// </summary>
        /// <param name="searchPaths">An ordered set of fully qualified paths which are searched when resolving assembly names.</param>
        ...
        static Hash32 hash(IEnumerable<string> src)
        {
            var dst = Hash32.Zero;
            sys.iter(src, item => dst |= sys.hash(item));
            return dst;
        }

        public override int GetHashCode() =>
            sys.hash(BaseDirectory) | hash(SearchPaths);
