...
namespace Z0
{
    ...
    public class CsvTableReader<T> : IDisposable
    {
        ...
        public CsvTableReader(FilePath src, bool header = true)
        {
        ...
        public CsvTableReader(FilePath src, RowParser<T> parser, bool header = true)
        {
            Stream = src.Utf8Reader();
            if(header)
            HeaderText = header ? Stream.ReadLine() : EmptyString;
