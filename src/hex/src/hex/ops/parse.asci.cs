...
partial struct Hex
{
    ...
    [MethodImpl(Inline), Op]
    public static bool parse(C c, out byte dst)
    {
        if(HexTest.scalar(c))
        {
    ...
    [MethodImpl(Inline), Op]
    public static bool parse(UpperCased @case, C c, out byte dst)
    {
        var result = false;
        if(HexTest.scalar(c))
        {
    ...
    [MethodImpl(Inline), Op]
    public static bool parse(LowerCased @case, C c, out byte dst)
    {
        var result = false;
        if(HexTest.scalar(c))
        {
    ...
    [MethodImpl(Inline), Op]
    public static bool parse(UpperCased @case, C c, out HexDigitValue dst)
    {
        var result = false;
    ...
    [MethodImpl(Inline), Op]
    public static bool parse(LowerCased @case, C c, out HexDigitValue dst)
    {
        var result = false;
        if(HexTest.scalar(c))
        {
    ...
    [MethodImpl(Inline), Op]
    public static bool parse(C c, out HexDigitValue dst)
    {
        if(HexTest.scalar(c))
        {
            dst = (HexDigitValue)((HexDigitCode)c - XF.MinScalarCode);
    ...
    //         if(SQ.whitespace(c) || HexTest.specifier(c))
    //             continue;
