...
partial struct Hex
{
    ...
    {
        if(HexTest.number(src))
            return (HexDigitValue)((HexSymFacet)src - NumberOffset);
        else
        {
            if(HexTest.test(UpperCase, src))
    ...
    public static HexDigitValue digit(LowerCased @case, char src)
        => HexTest.number(src) ? (HexDigitValue)((XF)src - NumberOffset) : (HexDigitValue)((XF)src - LetterOffsetLo);
    ...
    public static HexDigitValue digit(UpperCased @case, char src)
        => HexTest.number(src) ? (HexDigitValue)((XF)src - NumberOffset) : (HexDigitValue)((XF)src - LetterOffsetUp);
