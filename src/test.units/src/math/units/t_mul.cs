    ...
    public sealed class t_mul : UnitTest<t_mul>
    {
        ...
        public void mul_2x32u()
        {
            ...
            for(var i=0; i< RepCount; i++)
            {
                ...
                var lo = zUInt128.mullo(x,y);
                Claim.eq(x*y,lo);

                var hi = zUInt128.mulhi(x,y);
                Claim.eq(0,hi);

                zUInt128.mul(x,y, out uint a, out uint b);
                Claim.eq(lo, a);
                Claim.eq(hi, b);

                zUInt128.mul(z,MAX, out uint c, out uint d);
        ...
        public void mul_2x64u()
        {
            ...
            for(var i=0; i< RepCount; i++)
            {
                ...
                ClaimNumeric.nonzero(zUInt128.mulhi(z,MAX));

                zUInt128.mul(x,y, out ulong a, out ulong b);
                Claim.eq(lo, a);
                Claim.eq(hi, b);

                zUInt128.mul(z,MAX, out ulong c, out ulong d);
        ...
        public void mullo_2x64u()
        {
            for(var i=0; i< RepCount; i++)
            {
                ...
                Claim.eq(lo_ref(a,b), zUInt128.mullo(a,b));
            }
        }
    }
}