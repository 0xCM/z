...
namespace Z0
{
    public class ApiFiles
    {
        ...
        public static FileName filename(_ApiHostUri host, FileExt ext)
            => FS.file(host.Id.Format(), ext);

        public static FileName filename(_ApiHostUri host, FileExt a, FileExt b)
            => FS.file(text.concat(host.Id.Format(), a), b);

        [MethodImpl(Inline), Op]
        public static FolderName folder(_ApiHostUri host)
        ...
        public static FolderName folder(PartId part)
         ...
         [Op]
        public static FileName hostfile(_ApiHostUri uri, FileExt ext)
        ...
        public static bool IsHost(FileName src, _ApiHostUri host)
            => src.Name.Text.StartsWith(string.Concat(host.Part.Format(), Chars.Dot, host.HostName.ToLower(), Chars.Dot));

        [Op]
        public static FileName file(_ApiHostUri host, FileExt ext)
            => FS.file(string.Format("{0}.{1}", host.Part.Format(), host.HostName), ext);

        [Op]
        public static FileName file(_ApiHostUri host, string subject, FileExt ext)
            => FS.file(string.Format("{0}.{1}.{2}", host.Part.Format(), host.HostName, subject), ext);

        public static FileName file(_ApiHostUri host, FileKind kind)
            => file(host,kind.Ext());
