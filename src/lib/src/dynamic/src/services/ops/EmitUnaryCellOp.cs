...
namespace Z0
{
    partial class Dynop
    {
        ...
        internal static UnaryOp<T> EmitUnaryCellOp<T>(this IBufferToken dst, _OpIdentity id)
            where T : unmanaged
                => (UnaryOp<T>)dst.EmitUnaryCellOp(id,typeof(UnaryOp<T>), typeof(T));

        [Op]
        internal static CellDelegate EmitUnaryCellOp(this IBufferToken dst, _OpIdentity id, Type operatorType, Type operandType)
