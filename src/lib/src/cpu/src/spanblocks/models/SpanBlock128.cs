...
namespace Z0
{
    ...
    /// </summary>
    [SpanBlock(NativeTypeWidth.W128, SpanBlockKind.Sb128)]
        ...
        where T : unmanaged
    {
        ...
        [MethodImpl(Inline)]
        public static implicit operator Span<T>(SpanBlock128<T> src)
            => src.Data;

        [MethodImpl(Inline)]
        public static implicit operator ReadOnlySpan<T>(SpanBlock128<T> src)
            => src.Data;
    }
}