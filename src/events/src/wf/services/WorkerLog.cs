...
public class WorkerLog : IWorkerLog
{
    [MethodImpl(Inline), Op]
    public static IWorkerLog open(LogSettings config)
        => new WorkerLog(config);

    public static IWorkerLog open(FilePath dst, bool delete = true)
    {
        if(delete)
            dst.Delete();
        ...
        if(delete)
            errors.Delete();
        return open(new LogSettings(dst, errors));
    }
