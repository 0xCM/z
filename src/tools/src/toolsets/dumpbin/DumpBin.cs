    ...
    public partial class DumpBin : ToolService<DumpBin>
    {
        ...
        internal class Scripts
        {
            public static ToolScript DumpObj(FilePath input, IDbArchive dst)
                => Tools.script(dst.Path("dump-obj",FileKind.Cmd), vars(input.FolderPath, input.FileName, dst.Root));

            public static ToolScript DumpDll(FilePath input, IDbArchive dst)
                => Tools.script(dst.Path("dump-dll",FileKind.Cmd), vars(input.FolderPath, input.FileName, dst.Root));

            public static ToolScript DumpExe(FilePath input, IDbArchive dst)
                => Tools.script(dst.Path("dump-exe",FileKind.Cmd), vars(input.FolderPath, input.FileName, dst.Root));

            public static ToolScript DumpLib(FilePath input, IDbArchive dst)
                => Tools.script(dst.Path("dump-lib",FileKind.Cmd), vars(input.FolderPath, input.FileName, dst.Root));
