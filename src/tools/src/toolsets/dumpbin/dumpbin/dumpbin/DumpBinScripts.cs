...
namespace Z0
{
    internal class DumpBinScripts
    {
        public static ToolScript DumpObj(FilePath input, IDbTargets dst)
            => ToolScript.define(dst.Path("dump-obj",FileKind.Cmd), vars(input.FolderPath, input.FileName, dst.Root));

        public static ToolScript DumpDll(FilePath input, IDbTargets dst)
            => ToolScript.define(dst.Path("dump-dll",FileKind.Cmd), vars(input.FolderPath, input.FileName, dst.Root));

        public static ToolScript DumpExe(FilePath input, IDbTargets dst)
            => ToolScript.define(dst.Path("dump-exe",FileKind.Cmd), vars(input.FolderPath, input.FileName, dst.Root));

        public static ToolScript DumpLib(FilePath input, IDbTargets dst)
            => ToolScript.define(dst.Path("dump-lib",FileKind.Cmd), vars(input.FolderPath, input.FileName, dst.Root));
