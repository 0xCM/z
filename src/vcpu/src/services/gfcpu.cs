...
namespace Z0
{
    ...
    public readonly struct gfcpu
    {
       ...
       public static Vector128<T> vsub<T>(Vector128<T> x, Vector128<T> y)
        ...
        {
            if(typeof(T) == typeof(float))
                return generic<T>(vcpu.vsub(v32f(x), v32f(y)));
            else if(typeof(T) == typeof(double))
                return generic<T>(vcpu.vsub(v64f(x), v64f(y)));
            else
                throw no<T>();
        }
       ...
       public static Vector256<T> vsub<T>(Vector256<T> x, Vector256<T> y)
        ...
        {
            if(typeof(T) == typeof(float))
                return generic<T>(vcpu.vnegate(v32f(src)));
            else if(typeof(T) == typeof(double))
                return generic<T>(vcpu.vnegate(v64f(src)));
            else
                throw no<T>();
        }
