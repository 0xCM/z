...
public struct value
{
    ...
    public static value<T> from<T>(ReadOnlySpan<byte> src)
        where T : unmanaged
            => new(sys.first(sys.recover<T>(src)));
    ...
    {
        if(size<T>() == 1)
            return new(sys.@as<T>(M.sll(uint8(a.Data), count)));
        else if(size<T>() == 2)
            return new(@as<T>(M.sll(uint16(a.Data), count)));
        else if(size<T>() == 4)
            return new(@as<T>(M.sll(uint32(a.Data), count)));
        else if(size<T>() == 8)
            return new(@as<T>(M.sll(uint64(a.Data), count)));
        else
            throw no<T>();
    }
    ...
    {
        if(size<T>() == 1)
            return new(@as<T>(M.srl(uint8(a.Data), count)));
        else if(size<T>() == 2)
            return new(@as<T>(M.srl(uint16(a.Data), count)));
        else if(size<T>() == 4)
            return new(@as<T>(M.srl(uint32(a.Data), count)));
        else if(size<T>() == 8)
            return new(@as<T>(M.srl(uint64(a.Data), count)));
        else
            throw no<T>();
    }
    ...
    {
        if(size<T>() == 1)
            return new(@as<T>(M.add(uint8(a.Data), uint8(b.Data))));
        else if(size<T>() == 2)
            return new(@as<T>(M.add(uint16(a.Data), uint16(b.Data))));
        else if(size<T>() == 4)
            return new(@as<T>(M.add(uint32(a.Data), uint32(b.Data))));
        else if(size<T>() == 8)
            return new(@as<T>(M.add(uint64(a.Data), uint64(b.Data))));
        else
            throw no<T>();
    }
    ...
    {
        if(size<T>() == 1)
            return new(@as<T>(M.sub(uint8(a.Data), uint8(b.Data))));
        else if(size<T>() == 2)
            return new(@as<T>(M.sub(uint16(a.Data), uint16(b.Data))));
        else if(size<T>() == 4)
            return new(@as<T>(M.sub(uint32(a.Data), uint32(b.Data))));
        else if(size<T>() == 8)
            return new(@as<T>(M.sub(uint64(a.Data), uint64(b.Data))));
        else
            throw no<T>();
    }
    ...
    {
        if(size<T>() == 1)
            return new(@as<T>(M.div(uint8(a.Data), uint8(b.Data))));
        else if(size<T>() == 2)
            return new(@as<T>(M.div(uint16(a.Data), uint16(b.Data))));
        else if(size<T>() == 4)
            return new(@as<T>(M.div(uint32(a.Data), uint32(b.Data))));
        else if(size<T>() == 8)
            return new(@as<T>(M.div(uint64(a.Data), uint64(b.Data))));
        else
            throw no<T>();
    }
    ...
    {
        if(size<T>() == 1)
            return new(@as<T>(M.mod(uint8(a.Data), uint8(b.Data))));
        else if(size<T>() == 2)
            return new(@as<T>(M.mod(uint16(a.Data), uint16(b.Data))));
        else if(size<T>() == 4)
            return new(@as<T>(M.mod(uint32(a.Data), uint32(b.Data))));
        else if(size<T>() == 8)
            return new(@as<T>(M.mod(uint64(a.Data), uint64(b.Data))));
        else
            throw no<T>();
    }
    ...
    {
        if(size<T>() == 1)
            return new(@as<T>(M.inc(uint8(a.Data))));
        else if(size<T>() == 2)
            return new(@as<T>(M.inc(uint16(a.Data))));
        else if(size<T>() == 4)
            return new(@as<T>(M.inc(uint32(a.Data))));
        else if(size<T>() == 8)
            return new(@as<T>(M.inc(uint64(a.Data))));
        else
            throw no<T>();
    }
    ...
    {
        if(size<T>() == 1)
            return new(@as<T>(M.dec(uint8(a.Data))));
        else if(size<T>() == 2)
            return new(@as<T>(M.dec(uint16(a.Data))));
        else if(size<T>() == 4)
            return new(@as<T>(M.dec(uint32(a.Data))));
        else if(size<T>() == 8)
            return new(@as<T>(M.dec(uint64(a.Data))));
        else
            throw no<T>();
    }
