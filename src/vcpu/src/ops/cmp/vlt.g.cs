...
partial class vgcpu
{
    ...
    [MethodImpl(Inline), Op, Closures(Integers)]
    public static Vector512<T> vlt<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
            => vlt_u(x,y);
    ...
    [MethodImpl(Inline)]
    static Vector512<T> vlt_u<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
    {
        if(typeof(T) == typeof(byte))
            return generic<T>(vcpu.vlt(v8u(x), v8u(y)));
    ...
    [MethodImpl(Inline)]
    static Vector512<T> vlt_i<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
    {
        if(typeof(T) == typeof(sbyte))
                return generic<T>(vcpu.vlt(v8i(x), v8i(y)));
