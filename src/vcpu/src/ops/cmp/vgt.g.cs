...
partial class vgcpu
{
    ...
    [MethodImpl(Inline), Gt, Closures(Integers)]
    public static Vector512<T> vgt<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
            => vgt_u(x,y);
    ...
    [MethodImpl(Inline)]
    static Vector512<T> vgt_u<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
    {
        if(typeof(T) == typeof(byte))
            return generic<T>(vcpu.vgt(v8u(x), v8u(y)));
    ...
    [MethodImpl(Inline)]
    static Vector512<T> vgt_i<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
    {
        if(typeof(T) == typeof(sbyte))
                return generic<T>(vcpu.vgt(v8i(x), v8i(y)));
