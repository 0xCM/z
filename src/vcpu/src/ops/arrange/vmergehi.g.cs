...
partial class vgcpu
{
        ...
        where T : unmanaged
    {
        if(typeof(T) == typeof(sbyte))
                return generic<T>(vcpu.vmergehi(v8i(x), v8i(y)));
        else if(typeof(T) == typeof(short))
                return generic<T>(vcpu.vmergehi(v16i(x), v16i(y)));
        else if(typeof(T) == typeof(int))
                return generic<T>(vcpu.vmergehi(v32i(x), v32i(y)));
        else
                return generic<T>(vcpu.vmergehi(v64i(x), v64i(y)));
    }
    ...
    {
        if(typeof(T) == typeof(byte))
            return generic<T>(vcpu.vmergehi(v8u(x), v8u(y)));
        else if(typeof(T) == typeof(ushort))
            return generic<T>(vcpu.vmergehi(v16u(x), v16u(y)));
        else if(typeof(T) == typeof(uint))
            return generic<T>(vcpu.vmergehi(v32u(x), v32u(y)));
        else
            return generic<T>(vcpu.vmergehi(v64u(x), v64u(y)));
    }
