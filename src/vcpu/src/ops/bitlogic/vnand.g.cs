...
partial class vgcpu
{
    ...
    /// <param name="y">The right operand</param>
    [MethodImpl(Inline), Nand, Closures(Integers)]
    public static Vector512<T> vnand<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
            => vnand_u(x,y);
    ...
    [MethodImpl(Inline)]
    static Vector512<T> vnand_u<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
    {
        if(typeof(T) == typeof(byte))
            return generic<T>(vcpu.vnand(v8u(x), v8u(y)));
    ...
    [MethodImpl(Inline)]
    static Vector512<T> vnand_i<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
    {
        if(typeof(T) == typeof(sbyte))
            return generic<T>(vcpu.vnand(v8i(x), v8i(y)));
