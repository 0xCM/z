...
partial class vgcpu
{
    ...
    /// <typeparam name="T">The component type</typeparam>
    [MethodImpl(Inline), And, Closures(Integers)]
    public static Vector512<T> vand<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
            => vand_u(x,y);
    ...
    [MethodImpl(Inline)]
    static Vector512<T> vand_u<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
    {
        if(typeof(T) == typeof(byte))
            return generic<T>(vcpu.vand(v8u(x), v8u(y)));
    ...
    [MethodImpl(Inline)]
    static Vector512<T> vand_i<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
    {
        if(typeof(T) == typeof(sbyte))
            return generic<T>(vcpu.vand(v8i(x), v8i(y)));
