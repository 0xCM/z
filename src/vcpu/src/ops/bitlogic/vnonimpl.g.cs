...
partial class vgcpu
{
    ...
    [MethodImpl(Inline), NonImpl, Closures(Integers)]
    public static Vector512<T> vnonimpl<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
            => vnonimpl_u(x,y);
    ...
    [MethodImpl(Inline)]
    static Vector512<T> vnonimpl_u<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
    {
        if(typeof(T) == typeof(byte))
            return generic<T>(vcpu.vnonimpl(v8u(x), v8u(y)));
    ...
    [MethodImpl(Inline)]
    static Vector512<T> vnonimpl_i<T>(Vector512<T> x, Vector512<T> y)
        where T : unmanaged
    {
        if(typeof(T) == typeof(sbyte))
            return generic<T>(vcpu.vnonimpl(v8i(x), v8i(y)));
