...
partial class TypeNats
{
    ...
    public readonly struct Points<X0,X1,X2,T> : IIndex<Point<X0,X1,X2,T>>
        where X0 : unmanaged, ITypeNat
        where X1 : unmanaged, ITypeNat
        where X2 : unmanaged, ITypeNat
        where T : unmanaged
    {
        public readonly Index<Point<X0,X1,X2,T>> Data;

        [MethodImpl(Inline)]
        public Points(Point<X0,X1,X2,T>[] src)
        {
            Data = src;
        }

        public Point<X0,X1,X2,T>[] Storage
        {
            [MethodImpl(Inline)]
            get => Data.Storage;
        }
        ...
        public ref Point<X0,X1,X2,T> this[uint i]
        {
            [MethodImpl(Inline)]
            get => ref Data[i];
        }

        public ref Point<X0,X1,X2,T> this[int i]
        {
            [MethodImpl(Inline)]
            get => ref Data[i];
        }
        ...
        [MethodImpl(Inline)]
        public static implicit operator Points<X0,X1,X2,T>(Point<X0,X1,X2,T>[] src)
            => new (src);

        [MethodImpl(Inline)]
        public static implicit operator Points<X0,X1,X2,T>(Index<Point<X0,X1,X2,T>> src)
            => new (src);

        [MethodImpl(Inline)]
        public static implicit operator Point<X0,X1,X2,T>[](Points<X0,X1,X2,T> src)
            => src.Data;

        public static Points<X0,X1,X2,T> Empty => sys.empty<Point<X0,X1,X2,T>>();
    }    
}
