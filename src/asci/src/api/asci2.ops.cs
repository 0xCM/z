...
partial struct Asci
{
    ...
    [MethodImpl(Inline), Op]
    public static asci2 define(C a, C b)
        => new (AsciSymbols.pack(a,b));
    ...
    public static int length(asci2 src)
        => foundnot(AsciSymbols.search(sys.bytes(src), z8), src.Capacity);
    ...
    [MethodImpl(Inline), Op]
    public static asci2 spaced(N2 n)
        => new (0x2020);

    [MethodImpl(Inline)]
    public static asci2 init(N2 n, ReadOnlySpan<AsciCode> src)
        => new (first(recover<AsciCode,ushort>(src)));
