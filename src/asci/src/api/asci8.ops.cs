...
partial struct Asci
{
    ...
    {
        var decoded = vcpu.vmovzxbw(w256, vcpu.vbytes(w128, src.Storage));
    ...
    public static string decode(in asci8 src)
        => sys.@string(slice(recover<char>(sys.bytes(vcpu.vlo(vcpu.vmovzxbw(w256, vcpu.vbytes(w128, src.Storage))))), 0, src.Length));

    [MethodImpl(Inline), Op]
    public static void decode(N8 n, ReadOnlySpan<byte> src, Span<char> dst)
        => vcpu.vstore(vcpu.vmovzxbw(w256, vcpu.v8u(vcpu.vscalar(w128, @as<ulong>(sys.first(src))))), ref @as<ushort>(sys.first(dst)));
