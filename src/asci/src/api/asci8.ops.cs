...
namespace Z0
{
    ...
    partial struct Asci
    {
        ...
        public static string decode(in asci8 src)
            => sys.@string(slice(recover<char>(sys.bytes(vcpu.vlo(vpack.vinflate256x16u(vcpu.vbytes(w128, src.Storage))))), 0, src.Length));

        [MethodImpl(Inline), Op]
        public static void decode(N8 n, ReadOnlySpan<byte> src, Span<char> dst)
            => vcpu.vstore(vpack.vinflate256x16u(vcpu.v8u(vcpu.vscalar(w128, @as<ulong>(sys.first(src))))), ref @as<ushort>(sys.first(dst)));
        ...
        /// <param name="match">The value to match</param>
        [MethodImpl(Inline), Op]
        public static int index(asci8 src, AsciCode match)
            => AsciSymbols.search(@byte(edit(src)), src.Capacity, (byte)match);
