...
partial struct Asci
{
    ...
    public static void decode(asci16 src, ref char dst)
        => vcpu.vstore(vpack.vpmovzxbw(w256, src.Storage), ref @as<char,ushort>(dst));

    [MethodImpl(Inline), Op]
    public static void decode(N16 n, ReadOnlySpan<byte> src, Span<char> dst)
        => vcpu.vstore(vpack.vpmovzxbw(w256, vcpu.vload(w128,src)), ref @as<ushort>(sys.first(dst)));

    [MethodImpl(Inline), Op]
    public static string decode(asci16 src)
        => sys.@string(slice(recover<char>(sys.bytes(vpack.vpmovzxbw(w256, src.Storage))),0, src.Length));
    ...
    public static asci16 init(N16 n, ulong lo, ulong hi)
        => new (Vector128.Create(lo,hi).AsByte());

    [MethodImpl(Inline), Op]
    public static asci16 init(N16 n, AsciCode fill = AsciCode.Space)
        => new (vcpu.vbroadcast(w128, (byte)fill));
