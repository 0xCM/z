...
partial struct Asci
{
    ...
    public static asci4 encode(N4 n, char a, char b, char c)
        => new (AsciSymbols.pack((C)a, (C)b, (C)c, out var _ ));
    ...
    public static asci4 encode(N4 n, char a, char b, char c, char d)
        => new (AsciSymbols.pack((C)a, (C)b, (C)c, (C)d, out var _ ));
    ...
    {
        dst = asci4.Null;
        ref var storage = ref Unsafe.As<asci4,C>(ref dst);
        AsciSymbols.codes(src, count, ref storage);
        return dst;
    }
    ...
    {
        dst = default;
        AsciSymbols.codes(src, span<asci4,C>(ref dst));
        return dst;
    }
    ...
    /// <param name="match">The value to match</param>
    [MethodImpl(Inline), Op]
    public static int index(in asci4 src, C match)
        => AsciSymbols.search(@byte(edit(src)), src.Capacity, (byte)match);

    [MethodImpl(Inline), Op]
    public static asci4 spaced(N4 n)
        => new (0x20202020);

    [MethodImpl(Inline), Op]
    public static asci4 init(N4 n, ReadOnlySpan<C> src)
        => new (first(recover<C,asci4>(src)));
    ...
    [MethodImpl(Inline), Op]
    public static asci4 define(C a, C b, C c)
        => new (AsciSymbols.pack(a, b, c, out var _ ));
    ...
    [MethodImpl(Inline), Op]
    public static asci4 define(C c0, C c1, C c2, C c3)
        => new (AsciSymbols.pack(c0,c1,c2,c3, out var dst));
}
