...
partial struct Bitfields
{
    ...
    public static DataSize minsize<K>(ReadOnlySpan<BfSegDef<K>> src)
        where K : unmanaged
    {
    ...
    [MethodImpl(Inline), Op]
    public static DataSize minsize<T>(in BfDef<T> src)
        where T : unmanaged
    {
        var packed = 0u;
    ...
    [MethodImpl(Inline), Op]
    public static DataSize minsize(in BfDef src)
        => minsize(src.Segments);

    [MethodImpl(Inline), Op]
    public static DataSize minsize(ReadOnlySpan<BfSegDef> src)
    {
        var count = src.Length;
        var packed = 0u;
        for(var i=0; i<count; i++)
