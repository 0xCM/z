...
namespace Z0
{
    ...
    {
        public static Outcome<FileSplitInfo> split(IWfChannel channel, FileSplitSpec spec)
        {
            ...
            try
            {
                ...
                while(!reader.EndOfStream)
                {
                    ...
                    if(subcount >= spec.MaxLineCount)
                    {
                        writer.Flush();
                        writer.Dispose();
                        channel.EmittedFile(emitting, subcount);
                        path = NextPath(spec, ref splitcount);
                        paths.Add(path);
                        writer = path.Writer(spec.TargetEncoding);
                        linecount += subcount;
                        subcount = 0;
                        emitting = channel.EmittingFile(path);
                    }
                }
        ...
        static FilePath NextPath(in FileSplitSpec spec, ref uint part)
