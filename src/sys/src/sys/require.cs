...
namespace Z0
{
    partial class sys
    {
        ...
        {
            if(!invariant)
                @throw("The invariant, it failed", caller, line, file);
        }
        ...
        {
            if(src == null)
                @throw($"{caller} supplied a NULL value: {file}:line {line}");
            return src;
        }
            ...
            where N : unmanaged, ITypeNat
        {
            ...
            {
                @throw(string.Format(Pattern, n, src));
                return 0;
            }
        }
