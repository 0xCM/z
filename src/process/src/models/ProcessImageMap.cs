...
public record ProcessImageMap
{
    ...
    public readonly ReadOnlySeq<ProcessModule> Modules;

    [MethodImpl(Inline)]
    public ProcessImageMap(ProcessMemoryState state, ReadOnlySeq<ImageLocation> locations, ReadOnlySeq<ProcessModule> modules)
    {
        MemoryState = state;
