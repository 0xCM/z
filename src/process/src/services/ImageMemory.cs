...
namespace Z0
{
    ...
    public class ImageMemory
    {
        ...
        public static ExecToken dump(IWfChannel channel, ProcessAdapter src, IDbArchive dst)
        {
            ...
            channel.TableEmit(src.Modules.Select(x => x.Describe()), dst.Table<ProcessModuleInfo>());
        ...
        {
            var module = Process.GetCurrentProcess().Adapt().Modules.Where(m => Path.GetFileNameWithoutExtension(m.Path.Name) == procname).First;
        ...
        public static ProcessImageMap map(ProcessAdapter process)
        {
            ...
            return new ProcessImageMap(state, src, process.Modules);
        }
        ...
        public static void modules(ProcessAdapter src, IWfChannel channel, IDbArchive dst)
            => channel.TableEmit(src.Modules.Select(x => x.Describe()), uri(src, dst));

        public static ProcessModuleInfo describe(ProcessModule module)
        {
            var dst = new ProcessModuleInfo();
