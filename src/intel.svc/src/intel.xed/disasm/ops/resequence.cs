...
partial class XedDisasm
{
    ...
    public static Index<XedDisasmDetailBlock> resequence(Index<XedDisasmDetailBlock> src)
    {
        var dst = src.Sort();
        for(var i=0u; i<dst.Count; i++)
        {
            ...
            row.Seq = i;
            dst[i] = new XedDisasmDetailBlock(row, dst[i].SummaryLines, dst[i].Instruction);
        }
        return dst;
    }
}
