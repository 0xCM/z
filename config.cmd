...
set PublishLib=dotnet publish %ProjectPath% --output %ProjectDist% %ConfigOption% %FrameworkOption%
set PublishShell=dotnet publish %ProjectPath% --output %ProjectDist% %ConfigOption% %FrameworkOption% --publish-single-file
set DeployShell=%~dp0scripts\shell-deploy.cmd
set SlnPubRetract=rmdir %PublishedSln%
set RestoreProject=dotnet restore %ProjectPath% --packages %NuGetDeps% --use-current-runtime %VerbosityOption% --force-evaluate
set ProjectNugetConfig=%ProjectRoot%\nuget.config
set LocalRestore=dotnet restore %ProjectPath% --packages %NuGetDeps% --use-current-runtime %VerbosityOption% --force-evaluate --configfile %ProjectNugetConfig%
set ServeSite=http-server %SlnRoot% --port 48005 --ext txt --mimetypes %MimeTypes% --gzip --brotli -o
set RunToolOptions=--project %ProjectPath% --framework %FrameworkMoniker% %ConfigOption% --runtime %RuntimeMoniker% %VerbosityOption% --self-contained
